...
export default function TargetingRules() {
    ...
    const updateConfig = (updates: Partial<TargetingConfig>) => {
        autoSave(campaign.id, { targeting_config: { ...config, ...updates } })
    }
    ...
    const updatePageRule = (id: string, updates: Partial<PageRule>) => {
        updateConfig({
            pageRules: config.pageRules.map((r) =>
                r.id === id ? { ...r, ...updates } : r
            ),
        })
    }
        ...
        <div className="stack-lg">
            ...
            <section>
                ...
                <div className="segment-group">
                    {(["all", "new", "returning"] as VisitorOption[]).map((type) => (
                        <button
                            key={type}
                            onClick={() => updateConfig({ visitorType: type })}
                            className={`segment-btn ${config.visitorType === type ? "active" : ""}`}
                        >
                            {type}
                        </button>
                    ))}
            ...
            <section>
                ...
                <div className="segment-group">
                    {(["all", "desktop", "mobile", "tablet"] as DeviceOption[]).map((type) => (
                        <button
                            key={type}
                            onClick={() => updateConfig({ deviceType: type })}
                            className={`segment-btn ${config.deviceType === type ? "active" : ""}`}
                        >
                            {type}
                        </button>
                    ))}
            ...
            <section>
                <div className="row-between" style={{ marginBottom: 8 }}>
                    <h2>URL Rules</h2>
                    <button className="btn-link" onClick={addPageRule}>
                        + Add Rule
                    </button>
                </div>
                        ...
                        {config.pageRules.map((rule) => (
                            <div key={rule.id} className="row gap-4" style={{ padding: 6, border: "1px solid var(--framer-color-divider)", borderRadius: 6 }}>
                                <select
                                    ...
                                    onChange={(e) =>
                                        updatePageRule(rule.id, { exclude: e.target.value === "exclude" })
                                    }
                                >
                                ...
                                <select
                                    ...
                                    value={rule.type}
                                    onChange={(e) =>
                                        updatePageRule(rule.id, {
                                            type: e.target.value as PageRule["type"],
                                        })
                                    }
                                >
                                ...
                                <input
                                    type="text"
                                    value={rule.value}
                                    onChange={(e) => updatePageRule(rule.id, { value: e.target.value })}
                                    placeholder="/pricing"
                                    style={{ flex: 1, fontSize: 10 }}
                                />
                                <button className="btn-danger" onClick={() => removePageRule(rule.id)}>
                                    Remove
                                </button>
                            </div>
                        ))}
            ...
            <section>
                ...
                <input
                    type="text"
                    value={config.referrerSource}
                    onChange={(e) => updateConfig({ referrerSource: e.target.value })}
                    placeholder="google.com, facebook.com, etc."
                />
                <p style={{ fontSize: 10, marginTop: 4, color: "var(--framer-color-text-tertiary)" }}>
                    Leave empty to show for all referrers. Enter a domain to only show for visitors from that source.
                </p>
            </section>
            ...
            <section>
                ...
                <div className="segment-group" style={{ flexWrap: "wrap" }}>
                    {(
                        ...
                        ] as { value: FrequencyMode; label: string }[]
                    ).map((opt) => (
                        <button
                            key={opt.value}
                            onClick={() => updateConfig({ frequency: opt.value })}
                            className={`segment-btn ${config.frequency === opt.value ? "active" : ""}`}
                        >
                            {opt.label}
                        </button>
                    ))}
